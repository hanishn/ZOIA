...
| Generated filter audible cutoff sweep has bounded runtime evidence. | `npm run zoia:test:playwright:generated-patch-filter-audible-sweep -- --patch-root tests/workflow/generated-patches/filter-test-emulator --result-path tests/workflow/evidence/generated-patch-filter-audible-sweep/run-result.json` | Playwright audio evidence with disconnected-route and exaggerated-route controls | `status: pass`; `fixtureCount == 3`; `captureCount == 3`; `classification == audible-cutoff-sweep-supported`; generated route diff RMS `0.30199421202434645`, above measurable-sweep threshold; exaggerated seeded route diff RMS `0.37023452120977113`, above measurable-sweep threshold. | `tests\workflow\evidence\generated-patch-filter-audible-sweep\run-result.json` | Passing locally |
| Generated Reverb Lite validates, converts, and produces a measured wet tail. | `npm run zoia:test:playwright:generated-patch-reverb-semantics -- --graph-root tests/workflow/generated-patches/reverb-test --result-path tests/workflow/evidence/generated-patch-reverb-semantics/run-result.json` | Playwright audio evidence plus generated graph conversion | `status: pass`; `fixtureCount == 2`; `positiveTailCount == 1`; `negativeTailAbsentCount == 1`; converter emits a Reverb Lite emulator patch from the committed graph fixture; bypass negative control does not produce tail RMS. | `tests\workflow\evidence\generated-patch-reverb-semantics\run-result.json` | Passing locally |
| Reachable non-delay prompt classes have explicit runtime or blocker boundaries and cannot be mislabeled as delay or filter runtime support. | `npm run zoia:generate:patch:non-delay-boundary-controls -- --result-path tests/workflow/evidence/generated-patch-non-delay-boundary-controls/run-result.json` | Node orchestration inventory plus graph, conversion, blocker, and seeded mislabel controls | `status: pass`; `classCount == 8`; `passingClassCount == 8`; `inScopeRuntimeClassCount == 2`; `graphSupportedRuntimeUnsupportedCount == 1`; `validationBlockedCount == 4`; `selectionBlockedCount == 1`; `seededMislabelControlCount == 12`; `seededMislabelFailureDetectedCount == 12`; filter and Reverb Lite are the in-scope non-delay runtime classes and reference consumed browser/audio/control evidence; synth blocks conversion; sequencer, modulation-only, MIDI, and sampler block validation; unsupported prompt blocks selection. | `tests\workflow\evidence\generated-patch-non-delay-boundary-controls\run-result.json` | Passing locally |
...
| CV-to-filter-frequency scaling preserves static filter support. | `npm run zoia:test:playwright:generated-patch-filter-semantics -- --patch-root tests/workflow/generated-patches/filter-test-emulator --result-path tests/workflow/evidence/generated-patch-filter-runtime/filter-semantics-cv-scaling/run-result.json` | Playwright regression gate after accepted scaling change | Static gate records `status: pass`; `lowpassClassifiedCount == 1`; `bypassControlClassifiedCount == 1`; `highpassControlClassifiedCount == 1`; `captureCount == 3`; filter modulation route trace also passes after scaling. | `tests\workflow\evidence\generated-patch-filter-runtime\filter-semantics-cv-scaling\run-result.json` | Passing locally |
...
- The non-delay synth prompt must not become emulator-loadable delay or reverb runtime evidence; current expected result is converter `blocked` with `unsupported-generated-module` for `Synth Voice`.
...
reverb-lite-runtime-supported: runtime-reverb-lite-supported; positiveTailCount 1; negativeTailAbsentCount 1
...
## CV-To-Filter-Frequency Scaling Gate Definition

The CV-to-filter-frequency scaling gate records the accepted bounded scaling behavior and verifies that the static low-pass filter gate still passes.
...
- The generated audible-sweep evidence must render valid audio stimulus and pass the generated-route threshold.
- The static low-pass filter runtime gate must pass after scaling.
- The filter modulation route-trace gate must pass after scaling.
- The active claim boundary must remain limited to the bounded generated filter fixture.

Current result:

```text
generated-patch-filter-audible-sweep: status pass
generated-patch-filter-runtime\filter-semantics-cv-scaling: status pass
generated-patch-filter-modulation-semantics-cv-scaling: status pass
lowpassClassifiedCount: 1
bypassControlClassifiedCount: 1
highpassControlClassifiedCount: 1
captureCount: 3
