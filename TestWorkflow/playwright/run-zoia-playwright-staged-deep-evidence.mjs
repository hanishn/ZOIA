...
const MANIFEST_PATH = resolve(PROJECT_ROOT, "TestWorkflow", "canonical-patches", "Test_Modules.manifest.json");
const EVIDENCE_ROOT = resolve(PROJECT_ROOT, "TestWorkflow", "evidence", "q102-canonical-staged-deep-baseline");
  ...
  const fixtures = files
    ...
    .map((binFile) => {
      ...
      return {
        pairId,
        category,
        binPath: resolve(manifest.canonicalRoot, relativePath),
        jsonPath: jsonFile ? resolve(manifest.canonicalRoot, jsonFile.relativePath) : null,
