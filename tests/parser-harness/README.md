...
The promoted manifest preserves the original staged fixture provenance. Those staged binaries are not committed to this repository.

Full parser fixture execution requires the private staged fixture tree referenced by `fixtures/manifests/zoia-fixture-manifest.json`.
...
Use `npm run zoia:lint:no-magic` for the clone-safe parser harness check when private staged fixtures are not available.
