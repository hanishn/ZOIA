...
const DEFAULT_EVIDENCE_ROOT = resolve(PROJECT_ROOT, "tests/workflow/evidence/generated-patch-unmodified-modulated-timing");
const DEFAULT_RESULT_PATH = resolve(DEFAULT_EVIDENCE_ROOT, "run-result.json");
const EDGE_CHANNEL = "msedge";
const COMMAND = "npm run zoia:test:playwright:generated-patch-unmodified-modulated-timing";
const VIEWPORT = Object.freeze({ width: 1440, height: 1000 });
const JSON_SPACES = 2;
const SAMPLE_RATE = 44100;
const DURATION_SECONDS = 2.7;
const IMPULSE_AMPLITUDE = 0.75;
const IMPULSE_TIMES = Object.freeze([0.02, 0.72, 1.42]);
...
async function main() {
  ...
  for (const fileName of patchFiles) {
    ...
    const patch = await readJson(sourcePatchPath);
    const metadata = findRouteMetadata(patch);
    const baseId = fileName.replace(/\.patch\.json$/, "");
    for (const mode of ["original-default-expression", "original-high-expression", "original-high-expression-repeat", "muted-audio-input"]) {
      const fixturePath = resolve(evidenceRoot, "fixtures", `${baseId}.${mode}.patch.json`);
      await writeJson(fixturePath, patch);
      fixtures.push({
        id: `${baseId}-${mode}`,
        sourcePatchPath,
        ...
        patch,
        mode,
        expressionValue: mode === "original-default-expression" ? 0 : 1,
        mutedAudioInput: mode === "muted-audio-input",
        route: metadata
      });
    }
  }
  ...
  for (const fixture of fixtures) {
    const renderResult = await page.evaluate(async ({ patch, fixture, settings }) => {
      ...
      for (var t = 0; t < traceBuild.nodes.length; t++) {
        ...
        if (traceNode && traceNode.type === 'lfo' && fixture.route.lfoTimeRoute && traceNode.outputs && traceNode.outputs[fixture.route.lfoTimeRoute.srcBlock]) {
          traceNode.outputs[fixture.route.lfoTimeRoute.srcBlock].connect(traceCtx.destination);
          lfoTraceConnections++;
        }
        if (traceNode && traceNode.type === 'cport_exp_cv' && fixture.route.expressionFeedbackRoute && traceNode.outputs && traceNode.outputs[fixture.route.expressionFeedbackRoute.srcBlock]) {
          traceNode.outputs[fixture.route.expressionFeedbackRoute.srcBlock].connect(traceCtx.destination);
      ...
      for (var lt = 0; lt < lfoTraceBuild.nodes.length; lt++) {
        ...
        if (lfoNode && lfoNode.type === 'lfo' && fixture.route.lfoTimeRoute && lfoNode.outputs && lfoNode.outputs[fixture.route.lfoTimeRoute.srcBlock]) {
          lfoNode.outputs[fixture.route.lfoTimeRoute.srcBlock].connect(lfoTraceCtx.destination);
        }
      }
      ...
      for (var et = 0; et < expressionTraceBuild.nodes.length; et++) {
        ...
        if (expressionNode && expressionNode.type === 'cport_exp_cv' && fixture.route.expressionFeedbackRoute && expressionNode.outputs && expressionNode.outputs[fixture.route.expressionFeedbackRoute.srcBlock]) {
          expressionNode.outputs[fixture.route.expressionFeedbackRoute.srcBlock].connect(expressionTraceCtx.destination);
        }
      }
    ...
    const failures = [];
    assertCondition(failures, !fixture.route.problem, "fixture-derivation", "required delay route metadata could not be derived", fixture.route.problem);
    assertCondition(failures, Boolean(fixture.route.lfoTimeRoute), "route-under-test", "generated LFO-to-delay-time route missing", fixture.route);
    assertCondition(failures, Boolean(fixture.route.expressionFeedbackRoute), "route-under-test", "generated expression-to-feedback route missing", fixture.route);
    ...
    if (fixture.mutedAudioInput) {
      assertCondition(failures, (renderResult.stimulusEvents || []).length === 0, "audio-negative-control", "muted audio control unexpectedly connected impulse stimulus", renderResult.stimulusEvents);
      assertCondition(failures, renderResult.features.rms <= MAX_MUTED_AUDIO_RMS, "audio-negative-control", "muted audio control produced measurable output", { observed: renderResult.features.rms, threshold: MAX_MUTED_AUDIO_RMS });
    } else {
      ...
      assertCondition(failures, renderResult.features.rms > MAX_MUTED_AUDIO_RMS, "audio-output", "original patch produced no measurable signal", renderResult.features);
    }
    ...
    if (fixture.mutedAudioInput && failures.length === 0) classification = "muted audio input negative control";
    else if (signalPresent && allImpulsesMeasured && modulatedTiming && feedbackTail) classification = "stable measured modulation behavior";
    ...
    else if (signalPresent) classification = "signal-present only";
  ...
  const classificationLog = {
    schemaVersion: "zoia.generated-patch-unmodified-modulated-timing-classification-log.v1",
    generatedAt: nowIso(),
  ...
  const stimulusManifest = {
    schemaVersion: "zoia.generated-patch-unmodified-modulated-timing-stimulus-manifest.v1",
    generatedAt: nowIso(),
    patchRoot,
    renderSettings: {
      sampleRate: SAMPLE_RATE,
    ...
    routeScope: {
      ...
      preservedOriginalConnections: true,
      lfoTimeRouteRequired: true,
      expressionFeedbackRouteRequired: true,
      expressionValuesTested: [0, 1],
      runtimeInputNegativeControls: ["muted-audio-input"],
      deterministicBaselineInputModes: ["original-high-expression", "original-high-expression-repeat"]
    },
    thresholds: {
  ...
  const mainRecords = records.filter((record) => !record.mutedAudioInput);
  const mutedControls = records.filter((record) => record.mutedAudioInput);
  const stableMeasuredModulationCount = mainRecords.filter((record) => record.classification === "stable measured modulation behavior").length;
  const unstableModulationCount = mainRecords.filter((record) => record.classification === "measurable but unstable modulation behavior").length;
  const signalPresentOnlyCount = mainRecords.filter((record) => record.classification === "signal-present only").length;
  const blockedClassificationCount = records.filter((record) => record.classification.startsWith("blocked with exact cause:")).length;
  const mutedAudioControlClassifiedCount = mutedControls.filter((record) => record.classification === "muted audio input negative control").length;
  const feedbackTailCount = mainRecords.filter((record) => record.audioEvidence.features?.maxRepeatToFirstRatio >= MIN_TAIL_RATIO).length;
  const result = {
    schemaVersion: "zoia.generated-patch-unmodified-modulated-timing-evidence.v1",
    version: "0.4.0",
    revision: 1,
    ...
    summary: {
      ...
      classifiedCount,
      stableMeasuredModulationCount,
      unstableModulationCount,
      signalPresentOnlyCount,
      blockedClassificationCount,
      feedbackTailCount,
      mutedAudioControlClassifiedCount,
    ...
    claimBoundaries: {
      originalGeneratedPatchTimingClassificationClaim: assertionFailures.length === 0,
      originalGeneratedPatchParametersPreserved: true,
      originalGeneratedPatchConnectionsPreserved: true,
