...
const DEFAULT_EVIDENCE_ROOT = resolve(PROJECT_ROOT, "tests/workflow/evidence/generated-patch-unmodified-timing-negative-controls");
const DEFAULT_RESULT_PATH = resolve(DEFAULT_EVIDENCE_ROOT, "run-result.json");
const EDGE_CHANNEL = "msedge";
const COMMAND = "npm run zoia:test:playwright:generated-patch-unmodified-timing-negative-controls";
...
function deriveCorruptedFixture(sourcePatch, mode) {
  ...
  patch.labels = Array.from(new Set([...(patch.labels || []), "unmodified-timing-negative-control", mode]));
  patch.description = `${patch.description || ""} Unmodified timing classifier negative control: ${mode}.`.trim();
...
async function main() {
  ...
  for (const fileName of patchFiles) {
    ...
    for (const mode of ["remove-lfo-time-route", "remove-expression-feedback-route", "disable-feedback-sources"]) {
  ...
  const classificationLog = {
    schemaVersion: "zoia.generated-patch-unmodified-timing-negative-controls-classification-log.v1",
    generatedAt: nowIso(),
  ...
  const stimulusManifest = {
    schemaVersion: "zoia.generated-patch-unmodified-timing-negative-controls-stimulus-manifest.v1",
    generatedAt: nowIso(),
    patchRoot,
    renderSettings: {
      sampleRate: SAMPLE_RATE,
    ...
    routeScope: {
      ...
      corruptionModes: ["remove-lfo-time-route", "remove-expression-feedback-route", "disable-feedback-sources"],
  ...
  const result = {
    schemaVersion: "zoia.generated-patch-unmodified-timing-negative-controls-evidence.v1",
    version: "0.4.0",
    revision: 1,
