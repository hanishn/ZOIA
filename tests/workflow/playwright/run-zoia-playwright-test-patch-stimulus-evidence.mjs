...
const EVIDENCE_ROOT = resolve(PROJECT_ROOT, "tests/workflow", "evidence", "v0.4-test-patch-stimulus");
const EDGE_CHANNEL = "msedge";
const COMMAND = "npm run zoia:verify:test-patches:stimulus";
...
const MAX_ATTEMPTS_PER_FIXTURE = 2;
...
async function loadManifests() {
  ...
  targets.criteria.sampleDurationMs = Math.max(targets.criteria.sampleDurationMs, 6000);
  ...
  return {
    canonical,
    targets,
    fixtures: targets.targets.filter((target) => target.expectedStimulus === "keyboard-midi-cv").map((target) => {
      ...
      return { ...fixture, target: { ...target, expectedOutput: "signal" } };
    })
  };
}
...
async function runPatch(browser, metadata, fixture, attemptIndex = 1) {
  const basePatchDir = resolve(EVIDENCE_ROOT, sanitizePathPart(fixture.category), sanitizePathPart(fixture.pairId));
  const patchDir = attemptIndex === 1 ? basePatchDir : resolve(basePatchDir, `attempt-${attemptIndex}`);
  ...
  try {
    ...
    await recordStep("Apply deterministic keyboard MIDI stimulus", async () => {
      await page.waitForFunction(() => Boolean(window.ZOIA?.midiKeyboard && window.ZOIA?.sim?.running), null, { timeout: PATCH_TIMEOUT_MS });
      await page.evaluate(() => {
        window.__zoiaV04StimulusEvents = [];
        var note = 60;
        var stimulated = 0;
        var nodes = window.ZOIA.sim.nodes || [];
        for (var s = 0; s < nodes.length; s++) {
          if (nodes[s] && (typeof nodes[s].press === "function" || typeof nodes[s].toggle === "function")) {
            if (typeof nodes[s].press === "function") {
              nodes[s].press();
              window.__zoiaV04StimulusEvents.push({ type: "controlPress", moduleIndex: s, nodeType: nodes[s].type || null });
              stimulated++;
            } else if (typeof nodes[s].toggle === "function") {
              nodes[s].toggle();
              window.__zoiaV04StimulusEvents.push({ type: "controlToggle", moduleIndex: s, nodeType: nodes[s].type || null });
              stimulated++;
            }
          }
        }
        for (var i = 0; i < nodes.length; i++) {
          if (nodes[i] && typeof nodes[i].noteOn === "function") {
            nodes[i].noteOn(note, 127);
            window.__zoiaV04StimulusEvents.push({ type: "noteOn", note: note, moduleIndex: i, nodeType: nodes[i].type || null });
            stimulated++;
          }
        }
        if (stimulated === 0) {
          window.ZOIA.midiKeyboard.noteOn(note);
          window.__zoiaV04StimulusEvents.push({ type: "noteOn", note: note, moduleIndex: null, nodeType: "midiKeyboardFallback" });
        }
      });
    });
    ...
    const audioEvidence = await page.evaluate(async ({ criteria, target }) => {
      ...
      function applyStimulusPulse() {
        var note = 60;
        var nodes = window.ZOIA.sim.nodes || [];
        window.__zoiaV04StimulusEvents = window.__zoiaV04StimulusEvents || [];
        for (var s = 0; s < nodes.length; s++) {
          ...
          if (nodes[s] && typeof nodes[s].clockPulse === "function") {
            nodes[s].clockPulse();
            window.__zoiaV04StimulusEvents.push({ type: "clockPulse", moduleIndex: s, nodeType: nodes[s].type || null });
          }
          if (nodes[s] && typeof nodes[s].noteOn === "function") {
            nodes[s].noteOn(note, 127);
            window.__zoiaV04StimulusEvents.push({ type: "noteOn", note: note, moduleIndex: s, nodeType: nodes[s].type || null });
          }
        }
      }
      ...
      return {
        ...
        stimulusEvents: window.__zoiaV04StimulusEvents || [],
    ...
    const hasStimulus = Array.isArray(audioEvidence.stimulusEvents) && audioEvidence.stimulusEvents.some((event) => event.type === "noteOn");

    if (expectedSignal && hasMasterSignal) {
      status = PASS_STATUS;
      classification = "stimulus-signal-present";
    } else if (expectedSignal && hasLowFrequencyMasterSignal) {
      status = PASS_STATUS;
      classification = "stimulus-low-frequency-signal-present";
    } else if (expectedSignal && hasLowLevelWaveform) {
      status = PASS_STATUS;
      classification = "stimulus-low-level-waveform-present";
    } else if (fixture.pairId === "PROTOTYPE_H01_Mono_Synth") {
      status = CLASSIFIED_STATUS;
      classification = "prototype-composite-control-stimulus-unresolved";
    } else if (!expectedSignal && hasMasterSignal) {
      status = CLASSIFIED_STATUS;
      classification = "unexpected-signal-present";
    } else if (!hasStimulus) {
      status = FAIL_STATUS;
      classification = "stimulus-not-applied";
    } else if (!hasAudioInput && fixture.target.expectedStimulus === "test-tone") {
      status = FAIL_STATUS;
      classification = "no-source";
    } else if (staticAnalysis.oscillatorModuleCount > 0 && staticAnalysis.keyboardModuleCount > 0) {
      status = FAIL_STATUS;
      classification = "midi-cv-stimulus-no-signal";
    ...
    const record = {
      ...
      completedAt: nowIso(),
      attemptIndex,
      maxAttempts: MAX_ATTEMPTS_PER_FIXTURE,
      command: COMMAND,
      target: fixture.target,
      fixtureReference,
      staticAnalysis,
...
async function main() {
  ...
  for (const fixture of manifests.fixtures) {
    ...
    const attempts = [];
    for (let attemptIndex = 1; attemptIndex <= MAX_ATTEMPTS_PER_FIXTURE; attemptIndex++) {
      const attemptResult = await runPatch(browser, manifests, fixture, attemptIndex);
      attempts.push({
        attemptIndex,
        status: attemptResult.status,
        classification: attemptResult.classification,
        resultPath: attemptResult.artifacts?.resultPath || null
      });
      finalResult = attemptResult;
      if (attemptResult.status !== FAIL_STATUS) break;
    }
  ...
  const result = {
    schemaVersion: "zoia.v04-test-patch-stimulus-run-result.v1",
