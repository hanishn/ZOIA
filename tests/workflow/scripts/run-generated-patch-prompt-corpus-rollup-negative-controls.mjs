...
const EVIDENCE_ROOT = resolve(PROJECT_ROOT, "tests/workflow/evidence/generated-patch-prompt-corpus-rollup-negative-controls");
...
function runSeededCase(seedFlag, resultPath) {
  const command = spawnSync(process.execPath, [
    "tests/workflow/scripts/run-generated-patch-prompt-corpus-rollup.mjs",
    seedFlag,
    "--result-path",
    resultPath
  ], {
  ...
  return {
    scriptPath: "tests/workflow/scripts/run-generated-patch-prompt-corpus-rollup.mjs",
    seedFlag,
    resultPath,
...
async function main() {
  ...
  const seededCases = [
    {
      id: "mislabel-filter-as-delay",
      seedFlag: "--seed-mislabel-filter-as-delay",
      expectedSurface: "prompt-boundary"
    },
    {
      id: "mislabel-modulation-only-as-delay",
      seedFlag: "--seed-mislabel-modulation-only-as-delay",
      expectedSurface: "prompt-boundary"
    },
    {
  ...
  for (const seededCase of seededCases) {
    ...
    if (command.exitCode === 0) {
      failures.push({ surface: "command", message: "seeded prompt-corpus rollup command unexpectedly passed", evidence: command });
    }
    if (childResult?.status !== FAIL_STATUS) {
      failures.push({ surface: "child-result", message: "seeded prompt-corpus rollup did not fail", evidence: childResult?.summary || null });
    }
    if (!expectedFailureFound) {
      failures.push({
        surface: "expected-failure",
        message: "seeded prompt-corpus rollup did not record the expected failure surface",
  ...
  const result = {
    schemaVersion: "zoia.generated-patch-prompt-corpus-rollup-negative-controls.v1",
    version: "0.4.0",
    revision: 1,
    status: assertionFailures.length === 0 ? PASS_STATUS : FAIL_STATUS,
    command: "npm run zoia:generate:patch:prompt-corpus-rollup:negative-controls",
    startedAt,
    completedAt: nowIso(),
    summary: {
    ...
    claimBoundary: {
      ...
      promptCorpusPositiveClaim: false,
