...
const EVIDENCE_ROOT = resolve(PROJECT_ROOT, "tests/workflow/evidence/generated-patch-prompt-repeatability-rollup-negative-controls");
...
function runSeededCase(seedFlag, resultPath) {
  const command = spawnSync(process.execPath, [
    "tests/workflow/scripts/run-generated-patch-prompt-repeatability-rollup.mjs",
    seedFlag,
    "--result-path",
    resultPath
  ], {
  ...
  return {
    scriptPath: "tests/workflow/scripts/run-generated-patch-prompt-repeatability-rollup.mjs",
    seedFlag,
    resultPath,
...
async function main() {
  ...
  const seededCases = [
    ...
    {
      id: "missing-runtime-audio-evidence",
      seedFlag: "--seed-missing-runtime-evidence",
      expectedSurface: "audio-evidence"
    },
    {
      id: "mislabel-unsupported-as-delay",
      seedFlag: "--seed-mislabel-unsupported-as-delay",
  ...
  for (const seededCase of seededCases) {
    ...
    const command = runSeededCase(seededCase.seedFlag, seededResultPath);
    const childResult = existsSync(seededResultPath) ? await readJson(seededResultPath) : null;
    const assertionFailures = childResult?.assertionFailures || [];
    const expectedFailureFound = assertionFailures.some((failure) => failure.surface === seededCase.expectedSurface);
    const failures = [];
    if (command.exitCode === 0) {
      failures.push({ surface: "command", message: "seeded prompt-repeatability rollup command unexpectedly passed", evidence: command });
    }
    if (childResult?.status !== FAIL_STATUS) {
      failures.push({ surface: "child-result", message: "seeded prompt-repeatability rollup did not fail", evidence: childResult?.summary || null });
    }
    if (!expectedFailureFound) {
      failures.push({
        surface: "expected-failure",
        message: "seeded prompt-repeatability rollup did not record the expected failure surface",
        evidence: { expectedSurface: seededCase.expectedSurface, assertionFailures }
      });
    }
    records.push({
      ...seededCase,
  ...
  const result = {
    schemaVersion: "zoia.generated-patch-prompt-repeatability-rollup-negative-controls.v1",
    version: "0.4.0",
    revision: 1,
    status: assertionFailures.length === 0 ? PASS_STATUS : FAIL_STATUS,
    command: "npm run zoia:generate:patch:prompt-repeatability-rollup:negative-controls",
    startedAt,
    completedAt: nowIso(),
    summary: {
    ...
    claimBoundary: {
      ...
      promptRepeatabilityPositiveClaim: false,
