...
const EVIDENCE_ROOT = path.join(PROJECT_ROOT, 'tests/workflow/evidence/q107-ssl-shared-library-rebaseline-proof');
const RESULT_PATH = path.join(EVIDENCE_ROOT, 'run-result.json');
const ACTIVE_EXHIBIT = path.join(PROJECT_ROOT, 'products/zoia/index.html');
const ACTIVE_SCAN_ROOTS = [
  path.join(PROJECT_ROOT, 'products/zoia'),
  path.join(PROJECT_ROOT, 'tests/parser-harness'),
  path.join(PROJECT_ROOT, 'tests/workflow/playwright'),
...
function main() {
  ...
  const packageCandidateHashes = SSL_SHARED_CANDIDATES.map((candidate) => ({
    ...candidate,
    packageAsset: hashFile(path.join(PROJECT_ROOT, 'shared/ssl/zoia-shared-ssl/browser-assets', candidate.name)),
